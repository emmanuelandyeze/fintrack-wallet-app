interface SummaryCardProps {
	title: string;
	value: number;
	change: number;
	isCurrency?: boolean;
}

export const SummaryCard = ({
	title,
	value,
	change,
	isCurrency = true,
}: SummaryCardProps) => {
	const isPositive = change >= 0;
	const changeColor = isPositive
		? 'text-green-600'
		: 'text-red-600';
	const changeSign = isPositive ? '+' : '';

	return (
		<div className="bg-[#eaeff0] p-[28px] rounded-[20px] flex-1 min-w-[200px]">
			<div className="flex justify-between items-center mb-4">
				<h3 className="text-[#1B2528] text-[17px] font-bold">
					{title}
				</h3>
				<svg
					width="24"
					height="25"
					viewBox="0 0 24 25"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
					<path
						fill-rule="evenodd"
						clip-rule="evenodd"
						d="M5 10.25C3.89543 10.25 3 11.1454 3 12.25C3 13.3546 3.89543 14.25 5 14.25C6.10457 14.25 7 13.3546 7 12.25C7 11.1454 6.10457 10.25 5 10.25ZM10 12.25C10 11.1454 10.8954 10.25 12 10.25C13.1046 10.25 14 11.1454 14 12.25C14 13.3546 13.1046 14.25 12 14.25C10.8954 14.25 10 13.3546 10 12.25ZM17 12.25C17 11.1454 17.8954 10.25 19 10.25C20.1046 10.25 21 11.1454 21 12.25C21 13.3546 20.1046 14.25 19 14.25C17.8954 14.25 17 13.3546 17 12.25Z"
						fill="#1B2528"
					/>
				</svg>
			</div>
			<div className="flex flex-col items-baseline space-x-2">
				<p className="text-[34px] font-bold text-[#1B2528]">
					{isCurrency
						? `$${value.toLocaleString()}`
						: value.toLocaleString()}
				</p>
				<p
					className={`text-[13px] text-[#3E7383] font-semibold`}
				>
					{changeSign}
					{change}%
				</p>
			</div>
		</div>
	);
};
